...
2.  Crea un proyecto llamado **crudinstrumentos**.
3.  En el menú lateral, ve a **Firestore Database** y haz clic en "Crear base de datos" en modo de prueba.
...
1.  **Trigger (Disparador):** El usuario presiona "Guardar".
2.  **Validation Task:** El `ValidationAgent` comprueba que la edad sea mayor a 18.
3.  **Persistence Task:** Si la validación es exitosa, el flujo pasa al `PersistenceAgent` para inyectar en Firestore.
4.  **UI Sync:** Gracias al `StreamBuilder`, la interfaz se actualiza automáticamente sin refrescar.
